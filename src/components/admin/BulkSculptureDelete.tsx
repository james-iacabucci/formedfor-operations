...
export function BulkSculptureDelete() {
  const [deleteType, setDeleteType] = useState<'imported' | 'zip_import' | 'excel_import' | 'manual' | 'both'>('imported');
  ...
  const loadData = async () => {
    ...
    setCounts({
      imported: (zipCount || 0) + (excelCount || 0),
      zipImport: zipCount || 0,
      excelImport: excelCount || 0,
  ...
  const handleDelete = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('bulk-delete-sculptures', {
        body: {
          deleteType,
          batchIds: (deleteType === 'imported' || deleteType === 'zip_import' || deleteType === 'excel_import') && selectedBatches.length > 0 
            ? selectedBatches 
            : undefined,
        }
      });
  ...
  const showBatchSelector = deleteType === 'imported' || deleteType === 'zip_import' || deleteType === 'excel_import';
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-5 gap-4">
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm">ZIP Import</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold">{counts.zipImport}</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm">Excel Import</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold">{counts.excelImport}</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm">All Imported</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <div className="space-y-2">
            ...
            <Select value={deleteType} onValueChange={(v: any) => setDeleteType(v)}>
              ...
              <SelectContent>
                <SelectItem value="zip_import">ZIP Imported Only</SelectItem>
                <SelectItem value="excel_import">Excel Imported Only</SelectItem>
                <SelectItem value="imported">All Imported (ZIP + Excel)</SelectItem>
          ...
          {showBatchSelector && batches.length > 0 && (
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Leave unselected to delete all sculptures of the selected type
              </p>
                ...
                {batches.map(batch => (
                  <div key={batch.id} className="flex items-center gap-2 p-2 hover:bg-accent rounded">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium">{batch.file_name || 'Unnamed batch'}</p>
